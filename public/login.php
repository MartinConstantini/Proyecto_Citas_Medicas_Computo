<?php
?><!doctype html>
<html lang="es">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>citas medicas - login</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/css/bootstrap.min.css" rel="stylesheet">
    <link href="../assets/css/style.css" rel="stylesheet">
  </head>
  <body class="bg-soft-blue d-flex align-items-center justify-content-center vh-100">

    <div class="login-card shadow-lg rounded-4 p-4 p-md-5">
      <div class="text-center mb-4">
        <img src="../assets/img/logo.png" alt="logo" class="login-logo mb-2">
        <h1 class="h4 mb-0 text-primary-custom">sistema de citas medicas</h1>
        <p class="text-muted small mb-0">iniciar sesion</p>
      </div>

      <form id="form-login" class="mt-2">
        <div class="mb-3">
          <label class="form-label small text-muted">correo</label>
          <input type="email" name="email" class="form-control form-control-lg input-soft" placeholder="correo" required>
        </div>
        <div class="mb-3">
          <label class="form-label small text-muted">contrasena</label>
          <input type="password" name="password" class="form-control form-control-lg input-soft" placeholder="contrasena" required>
        </div>
        <button class="btn btn-primary w-100 btn-lg rounded-3 mt-2">entrar al sistema</button>
      </form>

      <p class="small mt-3 mb-0 text-center text-muted">
        no tienes cuenta?
        <a href="signup.php" class="link-primary">crear cuenta</a>
      </p>

      <div class="mt-4 d-flex align-items-center small text-muted">
        <img src="../assets/img/imagen1.png" alt="clinica" class="login-side-img me-2">
        <span>prototipo academico minimalista para gestion de citas medicas</span>
      </div>
    </div>

    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/js/bootstrap.bundle.min.js"></script>
    <script src="../assets/js/login.js"></script>
  </body>
</html>
